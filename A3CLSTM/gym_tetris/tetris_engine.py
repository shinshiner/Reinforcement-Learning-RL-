...
class GameState:
    ...
    def frame_step(self,input):
        ...
        if not self.isValidPosition(adjY=1):
            ...
            if cleared > 0:
                self.clean = max(self.clean, cleared)
                if cleared == 1:
            ...
            #reward = (self.height - self.getHeight()) / 100.0
        ...
        block = 0
        for yy in range(BOARDHEIGHT - 1, BOARDHEIGHT - self.height - 1, -1):
            for xx in range(0, BOARDWIDTH):
                if self.board[xx][yy] == BLANK:
                    block += 1
        self.old_block = self.new_block
        self.new_block = block
        if self.detailed_reward:
            reward += (self.old_block - self.new_block) * 0.3
    ...
    def getNewPiece(self):
        ...
        newPiece = {'shape': shape,
                    ...
                    'color': 2 }#random.randint(0, len(COLORS)-1)}
        return newPiece
    ...
    def addToBoard(self):
        ...
        for x in range(TEMPLATEWIDTH):
            for y in range(TEMPLATEHEIGHT):
                if PIECES[self.fallingPiece['shape']][self.fallingPiece['rotation']][y][x] != FAKE_BLANK:
    ...
    def isValidPosition(self,adjX=0, adjY=0):
        ...
        for x in range(TEMPLATEWIDTH):
            for y in range(TEMPLATEHEIGHT):
                ...
                if isAboveBoard or PIECES[self.fallingPiece['shape']][self.fallingPiece['rotation']][y][x] == FAKE_BLANK:
                    continue
